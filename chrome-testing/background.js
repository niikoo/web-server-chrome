...
function onchoosefolder(entry) {
  if (entry) {
    ...
    if (webapp) {
      ...
      webapp.fs = fs
      webapp.handlers = []
      webapp.add_handler(['.*', WSC.prepareHandler(DirectoryEntryHandler, fs, null)])
...
function backgroundAccept(sockInfo) {
  ...
  if (window.webapp && webapp.started) {
    console.info('App registrered an accept handler');
...
function launch(launchData) {
  ...
  //var page = 'index.html'
  var page = 'chrome-testing/polymer-ui/index.html'
...
function create_hidden() {
  ...
  if (app.opts && app.opts.optBackground && app.opts.optAllInterfaces) {
    ...
    chrome.app.window.create("chrome-testing/hidden.html",
      opts,
      oncreated)
  }
}
