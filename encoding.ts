...
export class EncoderBase {
  ...
  constructor () {
    ...
    this.encoders['utf-8'] = (options) => {
      return new this.UTF8Encoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['utf-8'] = (options) => {
      return new this.UTF8Decoder(options);
    };
    ...
    this.decoders['gbk'] = (options) => {
      return new this.GB18030Decoder(options);
    };
    ...
    this.encoders['gbk'] = (options) => {
      return new this.GB18030Encoder(options, true);
    };

    /** @param {{fatal: boolean}} options */
    this.encoders['gb18030'] = function (options) {
      return new this.GB18030Encoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['gb18030'] = function (options) {
      return new this.GB18030Decoder(options);
    };

    /** @param {{fatal: boolean}} options */
    this.encoders['big5'] = function (options) {
      return new this.Big5Encoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['big5'] = function (options) {
      return new this.Big5Decoder(options);
    };

    /** @param {{fatal: boolean}} options */
    this.encoders['euc-jp'] = function (options) {
      return new this.EUCJPEncoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['euc-jp'] = function (options) {
      return new this.EUCJPDecoder(options);
    };

    /** @param {{fatal: boolean}} options */
    this.encoders['iso-2022-jp'] = function (options) {
      return new this.ISO2022JPEncoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['iso-2022-jp'] = function (options) {
      return new this.ISO2022JPDecoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.encoders['shift_jis'] = function (options) {
      return new this.ShiftJISEncoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['shift_jis'] = function (options) {
      return new this.ShiftJISDecoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.encoders['euc-kr'] = function (options) {
      return new this.EUCKREncoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['euc-kr'] = function (options) {
      return new this.EUCKRDecoder(options);
    };
    // 14.3 utf-16be
    /** @param {{fatal: boolean}} options */
    this.encoders['utf-16be'] = function (options) {
      return new this.UTF16Encoder(true, options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['utf-16be'] = function (options) {
      return new this.UTF16Decoder(true, options);
    };

    // 14.4 utf-16le
    /** @param {{fatal: boolean}} options */
    this.encoders['utf-16le'] = function (options) {
      return new this.UTF16Encoder(false, options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['utf-16le'] = function (options) {
      return new this.UTF16Decoder(false, options);
    };
    /** @param {{fatal: boolean}} options */
    this.encoders['x-user-defined'] = function (options) {
      return new this.XUserDefinedEncoder(options);
    };
    /** @param {{fatal: boolean}} options */
    this.decoders['x-user-defined'] = function (options) {
      return new this.XUserDefinedDecoder(options);
    };
  }
  //
  // Utilities
  ...
  UTF8Decoder(options) {
    ...
    this.handler = function (stream, bite) {
      ...
      // 2. If byte is end-of-stream, return this.finished.
  ...
  UTF8Encoder(options) {
    ...
    this.handler = function (stream, code_point) {
      // 1. If code point is end-of-stream, return this.finished.
  ...
  SingleByteDecoder(index, options) {
     ...
     */
    this.handler = function (stream, bite) {
      // 1. If byte is end-of-stream, return this.finished.
      if (bite === end_of_stream)
        return this.finished;
  ...
  GB18030Decoder(options) {
    ...
    this.handler = function (stream, bite) {
      ...
      // second, and gb18030 third are 0x00, return this.finished.
   ...
   */
  GB18030Encoder(options, gbk_flag) {
    var fatal = options.fatal;
  ...
  ISO2022JPEncoder(options) {
    ...
    this.handler = (stream, code_point) => {
      ...
      // state is ASCII, return this.finished.
  ...
  UTF16Decoder(utf16_be, options) {
    ...
    this.handler = function (stream, bite) {
      ...
      // lead surrogate are null, return this.finished.
      if (bite === end_of_stream && utf16_lead_byte === null &&
        utf16_lead_surrogate === null) {
        return this.finished;
      }
...
export class TextDecoder {
  ...
  _decoder = null;
  /** @private @type {boolean} */
  _fatal = Boolean(this.options['fatal']);
  /** @private @type {boolean} */
  _ignoreBOM = Boolean(this.options['ignoreBOM']);
  encoding;
  fatal;
  ignoreBOM;
  /**
 * @constructor
 * @param {EncoderBase} enc: Encoding class
 * @param {string} _encoding The label of the encoding;
 *     defaults to 'utf-8'.
 * @param {Object} options
 */
  constructor (
    private enc: EncoderBase,
    private _encoding: string,
    private options: Object) {

    let encoding = (isNil(_encoding) ? String(_encoding) : DEFAULT_ENCODING);
    options = enc.ToDictionary(options);
    /** @private */
    this._encoding = enc.getEncoding(encoding);
    if (this._encoding === null || this._encoding['name'] === 'replacement')
      throw RangeError('Unknown encoding: ' + encoding);

    if (!enc.decoders[this._encoding['name']]) {
      throw Error('Decoder not present.' +
        ' Did you forget to include encoding-indexes.js?');

      if (!('encoding-indexes' in this.enc.global))
        return;
      enc.encodings.forEach(function (category) {
        ...
        category.encodings.forEach(function (encoding) {
          ...
          /** @param {{fatal: boolean}} options */
          this.decoders[name] = function (options) {
            return new enc.SingleByteDecoder(idx, options);
          };
          /** @param {{fatal: boolean}} options */
          this.encoders[name] = function (options) {
            return new enc.SingleByteEncoder(idx, options);
          };
        });
      });
  ...
  decode(input, options) {
    ...
    this.options = this.enc.ToDictionary(options);

    if (!this._streaming) {
      this._decoder = this.enc.decoders[this._encoding['name']]({ fatal: this._fatal });
      this._BOMseen = false;
    }
    ...
    return this.enc.codePointsToString(code_points);
  }
}
